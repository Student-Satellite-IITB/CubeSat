...
scal_R = 0.001
m_R = scal_R * m_Q2
m_F11 = np.dot(m_I, sqrtm(m_R)).dot(sqrtm(m_Q2)) + np.dot(sqrtm(m_Q2), sqrtm(m_R)).dot(m_I)
m_F11 = m_Q1 + 0.5 * m_F11
m_F11 = np.dot(m_I, sqrtm(m_R), sqrtm(m_F11))
m_F12 = np.dot(m_I, sqrtm(m_R), sqrtm(m_Q2))
m_F22 = m_Q1 + np.dot(m_I, sqrtm(m_R), sqrtm(m_Q2))
...
def control_law(x):
    gain = np.dot(np.linalg.inv(m_R), m_B.T).dot(m_F)
